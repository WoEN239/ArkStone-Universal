...
struct pidfParameters separatorPositonPidfParameters = {SEPARATOR_KP, SEPARATOR_KI, SEPARATOR_KD, 0.0, SEPARATOR_MAXI, SEPARATOR_MIN_SAMPLE_TIME, 0};

struct pidfState separatorPositionPidfState = {.0, .0, .0, .0, .0};

int8_t calculateSeparatorPower() {
    double u = pidfUpdate(&separatorPositonPidfParameters, &separatorPositionPidfState, separatorPositionError);
    return constrain(u, -100.0, 100.0);
}
